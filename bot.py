...
async def safe_search_gstin(gstin, semaphore=None):
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == BULK_GSTIN:
        # Bulk GSTIN mode: check every GSTIN and return ONLY ACTIVE GSTINs.
        ...
        await status.edit_text(
            f"⏳ {len(gstins)} GSTIN check ho rahe hain...\\n"
            f"⚡ Parallel requests: {REQUEST_CONCURRENCY}\\n"
            f"📄 Output me sirf ACTIVE GSTIN honge."
        )

        active_gstins = []
        errors = []

        # Process in batches to keep memory/task count controlled.
        batch_size = 100

        for start_index in range(0, len(gstins), batch_size):
            batch = gstins[start_index:start_index + batch_size]

            batch_results = await asyncio.gather(
                *(safe_search_gstin(gstin, semaphore) for gstin in batch)
            )

            for gstin, records, error in batch_results:
                # A GSTIN is considered active only when the API record explicitly
                # reports status == "Active".
                is_active = any(
                    clean(record.get("status")).lower() == "active"
            ...
            done = min(start_index + len(batch), len(gstins))
            await status.edit_text(
                f"⏳ Processing: {done}/{len(gstins)}\\n"
                f"✅ Active GSTINs: {len(active_gstins)}"
            )

        if not active_gstins:
            await status.edit_text(
                "❌ Koi ACTIVE GSTIN nahi mila."
            )
            return

        # TXT contains ONLY GSTIN numbers, one per line.
        output = "\\n".join(active_gstins)

        await status.edit_text(
            f"✅ Complete!\\n"
            f"Checked: {len(gstins)}\\n"
            f"🟢 Active: {len(active_gstins)}\\n"
            f"📄 TXT me sirf active GSTIN diye gaye hain."
        )
        await send_txt(update, output, "Active_GSTINs.txt")
        return
